...
_script='source <(curl -s  giteaz:3000/bal/bash-simplify/raw/commit/2ee8e5286f925e44d0081ac10beee497e0245ded/dir_util.sh) ; getCurScriptDirByConcat _dir _fn && echo "CurSctDir=${CurScriptDir}; CurSctFn=${CurScriptNm}" '  #和上一个脚本差别 是末尾的echo内容
_script="${_script/_dir/$(pwd)}"
_script="${_script/_fn/$0}"
eval "$(bash -c "$_script")"   #和上一个脚本差别 是 这里多了 eval
